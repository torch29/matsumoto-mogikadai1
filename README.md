...
10 ～ 30 代の社会人を対象とした、シンプルなインターフェースのフリマアプリです。
...
7. シーディングの実行でダミーデータが作られます（後述の「シーディング」項目をご参照ください）
...
2. [Stripe](https://stripe.com/jp) にアクセスし、[今すぐ試す]ボタンから、無料のアカウント登録/サインインをお願いします。（要メールアドレス。）
...
6. `.env` ファイルを開き、コピーした公開キーとシークレットキーを以下の欄に設定します。
   ```.env
...
- 全商品の一覧表示
  - トップページのアドレスは '/' です。出品された商品の全一覧が表示されます。
  - トップページ・商品詳細画面の閲覧は、会員未登録でも可能です。
- 会員登録について
  - 商品の出品・購入、コメント・いいね機能の利用には会員登録およびログインが必要です。
    ログインしていない場合、ログインを促す画面が表示されます。
  - 会員登録をすると、以下の機能が利用できます。
    - 商品へのいいね機能
    - 商品の出品・購入
    - マイページ機能（出品商品/購入商品/取引中商品の確認）
    - コメント機能
  - 商品購入後、出品者と購入者間での取引チャットを利用できます。
...
1. ユーザー
   ...
   ・かきく　くけこ　： test3@example.com（何もなし）
   　パスワード（３名共通）：12345678
   ```

2. データ

   - ログインすると、マイページから自身が出品した商品が確認できます。
